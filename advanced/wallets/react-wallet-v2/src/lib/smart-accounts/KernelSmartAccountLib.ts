import { Address, createPublicClient, Hex, http, PrivateKeyAccount, PublicClient, Transport } from 'viem'
...
import { BundlerActions, bundlerActions, BundlerClient,ENTRYPOINT_ADDRESS_V06,ENTRYPOINT_ADDRESS_V07, } from 'permissionless'
...
export class KernelSmartAccountLib implements EIP155Wallet {
  ...
  public constructor({ privateKey, chain, sponsored = false, entryPointVersion = 7 }: SmartAccountLibOptions) {
    this.chain = chain
  ...
  async init() {
    ...
    }) as unknown as KernelAccountClient<EntryPoint, Transport, Chain | undefined>
    this.client = client
