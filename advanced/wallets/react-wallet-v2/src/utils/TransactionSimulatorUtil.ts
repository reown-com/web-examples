...
const TransactionSimulatorUtil = {
  ...
  simulateAndCheckERC20Transfer: async (chainId:string, fromWalletAddress:string, calls: { to:string, value:string, data:string}[]) => {
    const projectId = process.env.NEXT_PUBLIC_PROJECT_ID || ''
    const provider = new providers.JsonRpcProvider(`https://rpc.walletconnect.org/v1?chainId=eip155:${chainId}&projectId=${projectId}`)
