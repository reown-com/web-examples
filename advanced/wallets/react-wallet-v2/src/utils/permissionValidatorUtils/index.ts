...
export function getPermissionScopeData(permissions:SingleSignerPermission[], chain:Chain):`0x${string}`{
   ...
   */
  let permittedScopeData:`0x${string}` = encodePacked(['uint8'],
      [permissions.length]
  );
  // considering permission is on same chain
  const chainIds = [BigInt(chain.id)]
  for (let i = 0; i < chainIds.length; ++i) {
    permittedScopeData = encodePacked(['bytes','uint64'],
       [ permittedScopeData,
        chainIds[i]]
    );
  }
