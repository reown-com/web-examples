...
import { type PaymentOption, type ContractInteraction, type DetailedPaymentOption, type CheckoutRequest, CheckoutErrorCode, CheckoutErrorMessages, createCheckoutError, Hex } from '@/types/wallet_checkout';
...
const WalletCheckoutUtil = {
    ...
    try {
      ...
      for (const payment of acceptedPayments) {
        ...
        if (payment.recipient) {
          ...
          // Validate matching chain IDs between asset and recipient
          if (!this.matchingChainIds(payment.asset, payment.recipient)) {
            throw new Error(`Chain ID mismatch between asset (${payment.asset}) and recipient (${payment.recipient})`);
          }
        }
  ...
  async prepareCheckoutRequest(
    ...
    try {
      ...
      if (!balance || !balance.balances) {
        throw createCheckoutError(
          CheckoutErrorCode.NO_MATCHING_ASSETS,
        );
      }
      ...
      const {feasibleDirectPayments, isUserHaveAtleastOneMatchingAssets} = this.processFeasibleDirectPayments(directPayments, balance);
      // Process contract payments
      const {feasibleContractPayments, isUserHaveAtleastOneMatchingAssets: validContractPayments} = await this.processFeasibleContractPayments(contractPayments, balance);
    ...
    const feasibleDirectPayments = directPayments
      .map(payment => {
        try {
          ...
          const { chainId, assetAddress, chainNamespace, assetNamespace } = this.getAssetDetails(payment.asset);
          ...
          const assetBalance = balance.balances.find(b => {
              if(assetNamespace === 'slip44'){
                return b.chainId.toLowerCase() === `${chainNamespace}:${chainId}`.toLowerCase() && b.address === undefined && b.symbol === 'ETH' && assetAddress === '60'
              }else {
                return (b.chainId.toLowerCase() === `${chainNamespace}:${chainId}`.toLowerCase() && 
                b.address?.toLowerCase() === `${chainNamespace}:${chainId}:${assetAddress}`.toLowerCase())
              }
            }
          );
          ...
          return {
            ...
            assetMetadata: {
              ...
              assetValue: (assetBalance.price * parseFloat(formatUnits(BigInt(paymentAmount), parseInt(assetBalance.quantity.decimals)))).toFixed(3),
  ...
  async processFeasibleContractPayments(
    ...
    let isUserHaveAtleastOneMatchingAssets = false;
    const feasibleContractPayments = await Promise.all(contractPayments.map(async payment => {
      try {
        const address = SettingsStore.state.eip155Address;
        const {asset, contractInteraction} = payment;
        const {chainId, assetAddress, chainNamespace, assetNamespace} = this.getAssetDetails(asset);
    ...
    const validPayments = feasibleContractPayments.filter((asset): asset is DetailedPaymentOption => asset !== null);
