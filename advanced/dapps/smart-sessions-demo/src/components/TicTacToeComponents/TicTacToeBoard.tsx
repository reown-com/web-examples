...
const GameStatus = memo(({
  ...
  if (winner) {
    message = <span className="text-green-600">{winner === 'X' ? 'You Won! 🎉' : 'System Won! 🤖'}</span>;
  } else if (isGameCompleted) {  // NEW branch
    message = <span className="font-semibold text-xl text-gray-600">Game Completed</span>;
  } else {
    message = <span className={`font-semibold text-xl ${isXNext ? 'text-green-600' : 'text-red-500'}`}>
...
const TicTacToeBoard = () => {
  ...
  const isBoardFull = gameState.board.every(cell => cell === "X" || cell === "O");
  const isGameCompleted = isBoardFull && !gameState.winner;

  const onMove = useCallback(async (gameId: string, position: number) => {
    // Early exit if the game is already completed
    if (gameState.winner || gameState.board.every(cell => cell === "X" || cell === "O")) {
      return;
    }
    ...
    try {
      ...
      const winnerInfo = movesCount >= 5 ? checkWinner([...updatedBoard]) : { winner: null, winningLine: null };
