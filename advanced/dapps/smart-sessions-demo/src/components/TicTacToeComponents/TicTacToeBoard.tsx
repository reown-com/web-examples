...
function TicTacToeBoard() {
  ...
  async function onMove(gameId: string, position: number) {
    ...
    try {
      ...
      setSmartSession((previous) => {
        if (!previous || !previous.grantedPermissions) {
          // Handle the case when previous state or grantedPermissions is undefined
          return previous; // Or return some other appropriate state
        }
        ...
        return {
          ...
          gameInfo: {
            gameState,
            gameStarted: previous.gameInfo?.gameStarted || false,
          },
        };
      });
