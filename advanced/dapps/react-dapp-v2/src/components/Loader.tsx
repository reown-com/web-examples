...
const SLoader = styled.svg<SLoaderStyleProps>`
  width: ${({ $size }) => `${$size}px`};
  height: ${({ $size }) => `${$size}px`};
...
const Loader = ({ size = 40, color = "lightBlue", rgb: rgbProp }: LoaderProps) => {
  const rgb = rgbProp || `rgb(${colors[color]})`;
  return (
