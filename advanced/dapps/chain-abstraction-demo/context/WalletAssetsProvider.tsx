...
export const WalletAssetsProvider: React.FC<{ children: React.ReactNode }> = ({
      ...
      if (capabilities.hasAssetDiscovery) {
        ...
        if (assetsObject) {
          ...
          if (chainAssets?.length > 0) {
            const tokenBalances = processAssetsToBalances(chainAssets, parseInt(chainId.toString()));
  ...
  }, [address, status, chainId, walletProvider, getAssetDiscoveryCapabilities, walletProviderType, getAssetsViaWalletService, getAssetsViaProvider, processAssetsToBalances]);
