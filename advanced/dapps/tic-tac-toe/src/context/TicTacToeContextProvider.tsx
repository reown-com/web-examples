...
export function TicTacToeContextProvider({ children }: { children: ReactNode }) {
  ...
  const isWalletConnecting = isConnecting || ['reconnecting', 'connecting'].includes(status)

  const [gameState, setGameState] = useLocalStorageState<GameState | undefined>(
    TICTACTOE_STATE,
    undefined
  )
  const [gameStarted, setGameStarted] = useLocalStorageState(TICTACTOE_STARTED, false)
