...
import { checkWinner, getBoardState, getGameIdFromReceipt, getTicTacToeAsyncPermissions, transformBoard, waitForTransaction } from '@/utils/TicTacToeUtils'
...
export function useTicTacToeActions() {
  ...
  async function handleSystemMove(gameId: string) {
    ...
    try {
      ...
      if(!systemMoveResponse.ok) {
        const errorMessage = systemMoveResponseData?.message || 'Error making system move'
        throw new Error(errorMessage)
      }
      ...
      console.log('Updated board after system move:', updatedBoardAfterSystemMove)

      // Check for a winner after the system's move
      const winnerInfoAfterSystemMove = checkWinner([...updatedBoardAfterSystemMove])
  ...
  async function startGame() {
    ...
    const approvedPermissions = await publicClient.grantPermissions(ticTacToePermissions)
