...
import { NEAR_TEST_CHAINS, TNearChain } from "../chains/near";
...
export default class NearWallet {
  ...
  async createTransactions({
    ...
    const provider = new providers.JsonRpcProvider(
      NEAR_TEST_CHAINS[chainId].rpc[0]
    );
  ...
  async signMessage({
    ...
    const { message, nonce, recipient, callbackUrl, state, accountId } =
      messageParams;
    const nonceArray = nonce; //Buffer.from(nonce);
...
export function decodeTransaction(transaction: Uint8Array | Object) {
  try {
    return nearTransactions.Transaction.decode(
      Buffer.from(transaction as Uint8Array)
    );
  } catch (error) {
    return nearTransactions.Transaction.decode(
      Buffer.from(Object.values(transaction as Object))
    );
  }
}
