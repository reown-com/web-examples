...
| **Uniswap Trading API** ✅ *(integrated)* | ✅ **Verified NOT gated (2026-07-10)**: fresh self-serve key applies `integratorFees` — 50 bps split confirmed in the quote's `aggregatedOutputs` and recipient present in `/swap` calldata. (The spec's "Fee is not enabled" 401 refers to the legacy per-key fee service) | `integratorFees: [{bips, recipient}]` | 500 bps | 0% documented | EOA, in-tx (Universal Router `PAY_PORTION`, output token) | ✅ | EVM |
