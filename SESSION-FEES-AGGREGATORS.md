...
| **Uniswap Trading API** ✅ *(integrated, E2E-verified on mainnet)* | ✅ **Verified NOT gated (2026-07-10)**: fresh self-serve key applies `integratorFees` — 50 bps split confirmed in the quote's `aggregatedOutputs` and recipient present in `/swap` calldata. (The spec's "Fee is not enabled" 401 refers to the legacy per-key fee service) | `integratorFees: [{bips, recipient}]` | 500 bps | 0% documented | EOA, in-tx (Universal Router `PAY_PORTION`, output token) | ✅ | EVM |
...
- **The per-request integrator-fee pattern is industry-standard**: 11 of 16
  ...
  **Uniswap** (empirically), plus 0x/Odos/OKX/LI.FI per docs; silently gated
  behind agreements — 1inch, Raydium (empirically). `wc_feeTerms`
...
2. **Uniswap Trading API** — very high wallet TNV (~$187M via Trust); DONE:
   integrated and **fee-verified on a self-serve key** (2026-07-10) — not
   gated, unlike 1inch.
